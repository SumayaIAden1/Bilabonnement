...
public class CarModelRepo {
    ...
    public void addCarModel(CarModel model) {
        String sql = "INSERT INTO car_model (brand, model_name, equipment_level, co2_emission) VALUES (?, ?, ?, ?)";
        template.update(sql,
                model.getBrand(),
                model.getModelName(),
    ...
    public void updateCarModel(CarModel model) {
        String sql = "UPDATE car_model SET brand = ?, model_name = ?, equipment_level = ?, co2_emission = ? WHERE model_id = ?";
        template.update(sql,
                model.getBrand(),
                model.getModelName(),
                model.getEquipmentLevel(),
